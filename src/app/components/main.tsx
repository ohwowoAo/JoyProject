...
const Main = () => {
  ...
  const [selectedCategory, setSelectedCategory] = useState("전체"); // 초기 선택은 '전체'
    ...
    <>
      ...
      <section className="w-[1325px] mx-auto mt-[100px] ">
        <div className="flex gap-4 font-bold text-2xl text-gray-600 mb-[25px]">
          <p>전체</p>
          <p onClick={() => setSelectedCategory("project")}>프로젝트</p>
          <p>스터디</p>
        </div>
            ...
            if (selectedCategory === "all" || selectedCategory === "project") {
              {
                Maindata.filter(
                  (data) =>
                    Array.isArray(data.tag) && data.tag.includes("project")
                ).map(function (data, i) {
                  return (
                    <div
                      key={i}
